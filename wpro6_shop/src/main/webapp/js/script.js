function zipCheck(){
	let url = "zipcheck.jsp?check=y"; // 우편번호 및 주소 찾기 
	window.open(url, "post", "toolbar=no, width=500, height=400, top=200, left=100, status=yes, scrollbars=yes menubar=yes");
}
...
function funcLogin() {
	if(loginForm.id.value === ""){
		alert("회원 ID를 입력해주세요.");
		loginForm.id.focus();
	}else if(loginForm.passwd.focus === "") {
		alert("회원 PASSWD를 입력해주세요.");
